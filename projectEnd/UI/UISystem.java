...
public class UISystem extends JFrame {
    ...
    private void selectZooUI() {
        ...
        try {
            ...
            if (zoos.isEmpty()) {
                ...
                zoos.forEach(zoo -> zooComboBox.addItem(zoo.getName()));
    ...
    public void addAnimalUI() {
        ...
        btnAdd.addActionListener(e -> {
            ...
            addAnimal(txtName, chkPredator, selectedCage.getId());
    ...
    private void editAnimalUI() {
        ...
        JComboBox<String> cmbAnimal = new JComboBox<>();
        JLabel lblName = new JLabel("Animal Name:");
        JTextField txtName = new JTextField();
        JLabel lblPredator = new JLabel("Is Predator:");
        JCheckBox chkPredator = new JCheckBox();
    ...
    private void deleteAnimalUI() {
        ...
        btnDelete.addActionListener(e -> {
            ...
            try {
                ...
                JOptionPane.showMessageDialog(this, "Error deleting animal: " + ex.getMessage(), "Database Error",
                        JOptionPane.ERROR_MESSAGE);
            }
    ...
    private void searchAnimalUI() {
        ...
        btnSearch.addActionListener(e -> {
            ...
            searchAnimal(selectedCage.getId(), searchTerm);
    ...
    public void displayAnimalsUI() {
        ...
        try {
            ...
            panel.add(new JLabel("Error fetching animals: " + ex.getMessage(), SwingConstants.CENTER),
                    BorderLayout.CENTER);
            ex.printStackTrace();
        }
    ...
    private void createZoo(String zooName) {
        ...
        try {
            ...
            if (nameExists) {
                JOptionPane.showMessageDialog(this, "Zoo with this name already exists.", "Error",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
    ...
    private void editZoo(Zoo selectedZoo, String newName) {
        try {
            List<Zoo> existingZoos = ZooRAR.getAllZoos();
            boolean nameExists = existingZoos.stream()
                    .anyMatch(zoo -> zoo.getName().equalsIgnoreCase(newName) && zoo.getId() != selectedZoo.getId());

            if (nameExists) {
                JOptionPane.showMessageDialog(this, "Zoo with this name already exist.", "Error",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
            selectedZoo.setName(newName);
            ZooRAR.updateZoo(selectedZoo);
            JOptionPane.showMessageDialog(this, "Zoo updated successfully!");
            mainMenuUI();

        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(this, "Error updating zoo: " + ex.getMessage(), "Database Error",
                    JOptionPane.ERROR_MESSAGE);
    ...
    private void deleteZoo(Zoo selectedZoo) {
        ...
        if (response == JOptionPane.YES_OPTION) {
            try {
                ...
                ZooRAR.deleteZoo(selectedZoo.getId());
                JOptionPane.showMessageDialog(this, "Zoo deleted successfully!");
                manageZooUI();
            } catch (SQLException ex) {
                JOptionPane.showMessageDialog(this, "Error deleting zoo: " + ex.getMessage(), "Database Error",
                        JOptionPane.ERROR_MESSAGE);
            }
        }
    }
    ...
    private void createCage(JTextField txtNumber, JComboBox<String> cmbSize, JTextField txtMaxAnimals) {
        try {
            ...
            if (maxAnimals <= 0) {
                JOptionPane.showMessageDialog(this, "Max Animals must be greater than 0.", "Error",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
            if (number <= 0) {
                JOptionPane.showMessageDialog(this, "Cage number must be greater than 0.", "Error",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            if (numberExists) {
                JOptionPane.showMessageDialog(this, "Cage with this number already exist", "Error",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            if (maxAnimals > maxSizeLimit) {
                JOptionPane.showMessageDialog(this,
                        "Max Animals cannot exceed " + maxSizeLimit + " for a " + size + " cage.", "Error",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            JOptionPane.showMessageDialog(this,
                    "Invalid number format. Please enter numeric values for Number and Max Animals.", "Error",
                    JOptionPane.ERROR_MESSAGE);
        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(this, "Error saving cage: " + ex.getMessage(), "Error",
                    JOptionPane.ERROR_MESSAGE);
        }
    }

    private void editCage(Cage selectedCage, JTextField txtNumber, JComboBox<String> cmbSize,
            JTextField txtMaxAnimals) {
        try {

            int number = !txtNumber.getText().trim().isEmpty() ? Integer.parseInt(txtNumber.getText().trim())
                    : selectedCage.getNumber();
            String size = (String) cmbSize.getSelectedItem() != null ? (String) cmbSize.getSelectedItem()
                    : convertSizeToString(selectedCage.getSize());
            int maxAnimals = !txtMaxAnimals.getText().trim().isEmpty()
                    ? Integer.parseInt(txtMaxAnimals.getText().trim())
                    : selectedCage.getMaxAnimals();
            ...
            List<Cage> existingCages = CageRAR.getCagesByZooId(currentZoo.getId());
            boolean numberExist = existingCages.stream()
                    .anyMatch(cage -> cage.getNumber() == number && cage.getNumber() != selectedCage.getNumber());
            if (numberExist) {
                JOptionPane.showMessageDialog(this, "Cage with this number already exist", "Error",
                        JOptionPane.ERROR_MESSAGE);
                return;
            ...
            int currentAnimalCount = AnimalRAR.getAnimalsByCageId(selectedCage.getNumber()).size();
            ...
            CageRAR.updateCage(selectedCage);
            JOptionPane.showMessageDialog(this, "Cage updated successfully!");
            manageCageUI();

        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(this, "Invalid input, please enter number", "Error",
                    JOptionPane.ERROR_MESSAGE);
        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(this, "Error updating cage: " + ex.getMessage(), "Database Error",
                    JOptionPane.ERROR_MESSAGE);
    ...
    private void deleteCage(Cage selectedCage) {
        int response = JOptionPane.showConfirmDialog(this,
                "Do you really want to delete cage with ID: " + selectedCage.getNumber() + " and size: "
                        + convertSizeToString(selectedCage.getSize()) + " ?",
                "Confirm Delete",
                JOptionPane.YES_NO_OPTION,
                JOptionPane.WARNING_MESSAGE);
        if (response == JOptionPane.YES_OPTION) {
            try {
                CageRAR.deleteCage(selectedCage.getId());
                JOptionPane.showMessageDialog(this, "Cage deleted successfully!");
                manageCageUI();
            } catch (SQLException ex) {
                JOptionPane.showMessageDialog(this, "Error deleting cage: " + ex.getMessage(), "Database Error",
                        JOptionPane.ERROR_MESSAGE);
            }
        }
    }
    ...
    private void addAnimal(JTextField txtName, JCheckBox chkPredator, int cageId) {
        ...
        try {
            Cage cage = CageRAR.getCageById(cageId);
            if (cage == null) {
            ...
            if (currentAnimalCount >= cage.getMaxAnimals()) {
                JOptionPane.showMessageDialog(this, "This cage is already at maximum capacity.", "Error",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            JOptionPane.showMessageDialog(this, "Error adding animal: " + ex.getMessage(), "Error",
                    JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void editAnimal(Animal selectedAnimal, JTextField txtName, JCheckBox chkPredator) {
        try {
            ...
            JOptionPane.showMessageDialog(this, "Error updating animal: " + ex.getMessage(), "Database Error",
                    JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void searchAnimal(int cageId, String searchText) {
        ...
        try {
            ...
            if (foundAnimals.isEmpty()) {
                panel.add(new JLabel("No animals found matching the search criteria.", SwingConstants.CENTER),
                        BorderLayout.CENTER);
            } else {
            ...
            JOptionPane.showMessageDialog(this, "Error searching for animals: " + ex.getMessage(), "Database Error",
                    JOptionPane.ERROR_MESSAGE);
        }
        JButton backButton = new JButton("Back to Manage Animals");
    ...
    private String formatCageInfo(Cage cage) {
        try {
            ...
            return "ID: " + cage.getNumber() + ", Size: " + convertSizeToString(cage.getSize()) + ", Animals: "
                    + animalCount + "/" + cage.getMaxAnimals();
        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(this, "Error getting animal count: " + ex.getMessage(), "Database Error",
                    JOptionPane.ERROR_MESSAGE);
            return "ID: " + cage.getNumber() + ", Size: " + convertSizeToString(cage.getSize()) + ", Animals: Error";
        }
    }
